...
$join = [
    db_prefix().'clients LEFT JOIN '.db_prefix().'contracts ON '.db_prefix().'contracts.client = '.db_prefix().'clients.userid LEFT JOIN '.db_prefix().'contracts_types ON '.db_prefix().'contracts_types.id = '.db_prefix().'contracts.contract_type',
...
// Filter by having contracts by stype
...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [
    ...
    db_prefix().'contracts_types.name', 
]);
...
foreach ($rResult as $aRow) {
    ...
    $row[] = ($aRow['name'] != '' ? $aRow['name'] : 'No Contract');
    
        // Custom fields add values
    foreach ($customFieldsColumns as $customFieldColumn) {
        $c_value_type = (strpos($customFieldColumn, 'date_picker_') !== false ? _d($aRow[$customFieldColumn]) : $aRow[$customFieldColumn]);
