...
<div class="modal-body">
   ...
   <div class="row">
      <div class="col-md-8 task-single-col-left">
         ...
         <div class="tasks-comments inline-block full-width simple-editor"<?php if(count($task->comments) == 0){echo ' style="display:none"';} ?>>
            ...
            <div id="task-comments" class="mtop10">
               <input type="hidden" id="comment_counter" name="comment_counter" value="<?php count($task->comments) ?>"/>
               <?php
                  $comments = '';
         ...
         <hr class="hr-10">
         <br/>
         <div id="call_log_outer">
                  <div id="call_log_inner">
                     <span><?php echo _l('create_call_log'); ?></span>&nbsp;&nbsp;
                     <div class="radio radio-primary radio-inline">
                        <input type="radio" value="1" id="call_log_1" name="create_call_log">
                        <label for="call_log_1"><?php echo _l('Yes'); ?></label>
                     </div>
                     <div class="radio radio-primary radio-inline">
                        <input type="radio" value="0" id="call_log_0" name="create_call_log">
                        <label for="call_log_0"><?php echo _l('No'); ?></label>
                     </div>
                  <br/>
                  <?php echo form_open_multipart('https://www.shopinitask.com/admin/call_logs/call_log?task=true',array('id'=>'calllog-form')) ;?>
                  <div id="call_log_button">
                     <br/>
                     <div class="col-md-12" style="padding:0px;">
                <div class="panel_s">
                    <div class="panel-body">
                        <h4 class="no-margin"><?php echo "Add New"; ?></h4>
                        ...
                        <div class="form-group select-placeholder">
                            ...
                            <select name="customer_type" id="customer_type" class="selectpicker" data-width="100%" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                                <option value=""></option>
                                <option value="lead" <?php if(isset($task->rel_type) && $task->rel_type == 'lead'){echo 'selected';} ?>><?php echo _l('cl_lead'); ?></option>
                                <option value="customer" <?php if(isset($task->rel_type) && $task->rel_type == 'customer'){echo 'selected';} ?>><?php echo _l('cl_customer'); ?></option>
                            </select>
                        </div>
                        <?php /*
                        <div class="form-group select-placeholder">
                        ...
                        <div class="form-group select-placeholder">
                            <?php echo render_select_with_input_group('rel_type',$rel_types,array('id','name'),'cl_type',$selected,'<a href="#" onclick="new_cl_type();return false;"><i class="fa fa-plus"></i></a>');
                        } else {
                        ...
                        ?>
                        <div id="call_purpose" class="hidden">
                        <?php $value = (isset($task->name) ? $task->name : $task->name); ?>
                        <?php echo render_input('call_purpose','call_purpose',$value); ?>
                        </div>
                        <div id="call_summary">
                        <?php $call_summary = isset($task->comments) ? $task->comments : ""; ?>
                        <input type="hidden" name="call_summary" value="<?php echo $call_summary ;?>" id="call_summary"/>
                        </div>
                        <input type="hidden" name="task_id" value="<?php echo $task->id; ?>" id="task_id"/>
              ...
              <!--   <?php 
                echo render_select_with_input_group('call_status',['Completed','Busy','No Answer','Cancelled','Failed'],array(),'call_log_direction',$selected,'<a href="#" onclick="new_call_direction();return false;"><i class="fa fa-plus"></i></a>');
                ?> -->
                <div class="col-md-12 start-calendar"  style="padding:0px;">
    ...
    <div class="col-md-12" id="cl_call_duration_div" style="padding:0px;">
    ...
    <div class="col-md-12">
        <div class="">
         ...
         <div class="text-right" id="save_task_button">
            <button type="button" class="btn btn-info save-cl">Save</button>
         </div>
      </div>
...
<div class="btn-bottom-pusher"></div>
      <div class="col-md-4 task-single-col-right">
         ...
         <h5 class="no-mtop task-info-created">
            ...
            <!-- <small class="text-dark"><?php echo _l('task_created_by',_dt($task->datefrom)); ?><br /> -->
            <small class="text-dark"><?php echo _l('task_created_by','<span class="text-dark">'.($task->is_added_from_contact == 0 ? get_staff_full_name($task->addedfrom) : get_contact_full_name($task->addedfrom)).'</span>'); ?><br />
            <small class="text-dark"><?php echo _l('task_created_at',_dt($task->dateadded)); ?>
            <?php //echo _l('task_created_by','<span class="text-dark">'.($task->is_added_from_contact == 0 ? get_staff_full_name($task->addedfrom) : get_contact_full_name($task->addedfrom)).'</span>'); ?> <!--<i class="fa fa-clock-o" data-toggle="tooltip" data-title="<?php //echo _l('task_created_at',_dt($task->dateadded)); ?>"></i></small>-->
            <br />
            <?php } else { ?>
         ...
         <div class="task_users_wrapper">
            <?php
               ...
               <span style="margin-left:10px;">'.html_escape($assignee['full_name']).'</span>';
               }
               if ($_assignees == '') {
         ...
         <div class="task_users_wrapper">
            <?php
               ...
               <span>'. html_escape($follower['full_name']).'</span></span><br/>';
               }
               if ($_followers == '') {
...
<script>
   ...
   $('#task-modal').find('.gpicker').googleDrivePicker({ onPick: function(pickData) {
    ...
    var is_call_log_exist = "<?php echo($task->call_log_id != NULL ? 1 : 0); ?>";
   if(is_call_log_exist == 1){
      $("#call_log_outer").hide();
   }
   $("#call_log_button").hide();
   $("#save_task_button").hide();
   
   

   $('#call_log_1').click(function(){
      checkComments();
   });
   ...
   function checkComments(){
      ...
      return $.ajax({
        url: admin_url+'tasks/get_task_comments',
        method: 'POST',
        data: {id: task_id}
      });
      }
      ...
      .then(function(data) { // Resolve
         ...
         if(data.count > 0){
            $("#call_summary").val(data.content[0].content);
            $("#call_log_button").show();
            $("#save_task_button").show();
            return true;
         }
      else{
         alert("Please Add Comment First");
         $('#call_log_1').prop('checked', false);
         return false;
      }
   }, function(){ // Reject!
   ...
   $("#call_log_0").click(function()
   { 
      if($('#call_log_0').is(':checked')){
         $("#call_log_button").hide();
         $("#save_task_button").hide();
      }
   });
