...
<div class="content">
   <div class="row">
               ...
               <h4 class="no-margin"><?php echo $member->firstname . ' ' . $member->lastname; ?>
                  ...
                  <small> - <?php echo _l('last_active'); ?>:
                        <span class="text-has-action" data-toggle="tooltip" data-title="<?php echo _dt($member->last_activity); ?>">
                              <h4><?php echo time_ago($member->last_activity); ?></h4>
                        </span>
      ...
      <div class="panel_s">
            <div class="panel-body">
               <h4 class="no-margin">
                  <?php echo _l('last_login_reports'); ?>
               </h4>
               <hr class="hr-panel-heading" />
                  <table class="table dt-table scroll-responsive">
                     ...
                     <tbody>
                        <?php
                           $total_logged_time = 0;
                           foreach($staff_login_data as $t){ ?>
                           <?php if($t['staffid'] == $member->staffid){?>
                        <tr>
                           <td><?php echo date("d-m-Y H:i:s",strtotime($t['login_time'])); ?></td>
                           <td><?php echo date("d-m-Y H:i:s",strtotime($t['last_login_time'])); ?></td>
                           <td>
                              <?php 
                              if(isset($t['last_login_time'])){
                                 $time1 =  date("H:i:s",strtotime($t['login_time']));
                                 $time2 =  date("H:i:s",strtotime($t['last_login_time']));
               ...
               <hr class="hr-panel-heading" />
                  <table class="table dt-table scroll-responsive">
                     ...
                     <tbody>
                        <?php
                           $total_logged_time = 0;
                           foreach($staff_login_details as $data){ ?>
                           <?php if($t['staffid'] == $member->staffid){?>
                        <tr>
                           <td><?php echo date("d-m-Y H:i:s",strtotime($data['time_in'])); ?></td>
                           <td><?php echo date("d-m-Y H:i:s",strtotime($data['time_out'])); ?></td>
                           <td><?php 
                           $last = explode("/", $data['url']);
                           echo (ucfirst(isset($last[5]) ? $last[5] : $last[4]));?></td>
                           <td>
                              <?php 
                              if(isset($data['time_in'])){
                                 $time1 =  date("H:i:s",strtotime($data['time_in']));
                                 $time2 =  date("H:i:s",strtotime($data['time_out']));

                                 $diff = abs(strtotime($time1) - strtotime($time2));
                                 $tmins = $diff/60;
                                 $hours = floor($tmins/60);
                                 $mins = $tmins/60;
                                 $seconds = ceil($mins/60);
                                 
                                 echo "<b> $hours </b> hours & <b> ".round($mins,2)." </b> minutes &</b> $seconds seconds</b>";
                              }
                              ?>
                           </td>
      ...
      <div class="panel_s">
            <div class="panel-body">
               <h4 class="no-margin">
                  <?php echo _l('history_log_report'); ?>
               </h4>
               <hr class="hr-panel-heading" />
                  <table class="table dt-table scroll-responsive">
                     <thead>
                     ...
                     <tbody>
                        <?php
                           $total_logged_time = 0;
                           foreach($history_activity_log as $data){ ?>
                           <?php if(!(str_contains($data['description'], 'Email Sent') && $member->staffid == 1)){?>
                        <tr>
                           <td><?php echo $data['id']; ?></td>
                           <td>
                              <?php //if(!(str_contains($data['description'], 'Email Sent'))){
