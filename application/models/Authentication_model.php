...
class Authentication_model extends App_Model
{
    ...
    private function update_login_info($user_id, $staff)
    {
        ...
        $this->insert_login_info_staff($user_id);
    ...
    //Adding Data Into Seperate Table for Login Reports
    private function insert_login_info_staff($user_id){
        ...
        $this->save_data_in_session($insert_id,$user_id,$data);
        
    }

    private function save_data_in_session($insert_id,$user_id,$data){
        $this->load->library('session');
        $currentURL = current_url();

        $session_data_loging_details = array(
            'insert_id'  => $insert_id,
            'login_time'=>date('Y-m-d H:i:s'),
            'last_login_time'=>date('Y-m-d H:i:s'),
        ...
        if(isset($_SESSION['insert_id'])){
            $this->session->unset_userdata($session_data_loging_details);//Unsert Session If it's already there
        }
        ...
        $this->session->set_userdata($session_data_loging_details);//Adding SESSION ID in Session
    }
