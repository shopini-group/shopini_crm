...
class Tasks extends AdminController
{
    ...
    public function get_task_data($taskid, $return = false)
    {
        ...
        $data['rel_types'] = $this->call_logs_model ? $this->call_logs_model->get_rel_types(): '';
        $data['call_directions'] = $this->call_logs_model  ? $this->call_logs_model->get_call_directions() : '';
    ...
    public function get_task_comments(){
        ...
        $task_comments = $this->tasks_model->get_task_comments($id);
        $number_of_comments = count($task_comments);
        
        if($number_of_comments == 0){
            $data =  ["count"=>0,"content"=>NULL];
        }else if($number_of_comments > 0){
            $data = ["count"=>$number_of_comments,"content"=>$task_comments];
        }
