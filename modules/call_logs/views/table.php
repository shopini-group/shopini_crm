...
//$join = hooks()->apply_filters('call_logs_table_sql_join', $join);
    array_push($filter, 'AND ('.db_prefix().'call_logs.call_direction="1" OR '.db_prefix().'call_logs.call_direction="3"'.')');
        ...
        if ($aColumns[$i] == 'call_purpose') {
            ...
            $_data .= '</div>';
                ...
                if($_data != ''){
                    ...
                    $task =  $CI->tasks_model->get($_data);
                    $_data = '<a href='.admin_url('tasks/view/'.$task->id).'>'.$task->name.'</a>';
                }
