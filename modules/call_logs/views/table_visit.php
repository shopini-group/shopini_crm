...
//Filter by Visit
array_push($filter, 'AND ('.db_prefix().'call_logs.call_direction="2"'.')');
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        }else if($aColumns[$i] == 'meeting_start_time'){
            $_data = $aRow['meeting_start_time'];
        }else if($aColumns[$i] == 'meeting_duration'){
            $_data = $aRow['meeting_duration'];
        }
        $row[] = $_data;
    }
    ob_start();
