...
class Call_logs extends AdminController
{
    ...
    public function visit_log_relations($clientid, $customer_type)
    {
        $data['clientid'] = $clientid;
        $data['customer_type'] = $customer_type;
        $this->app->get_table_data(module_views_path('call_logs', 'visit_log_relations'), $data);
    }
        ...
        if ($this->input->post()) {
            ...
            if ($id == '') {
                ...
                if ($id) {
                    ...
                    if($_GET['task'] == true){
                        $data['call_log_id'] = $id;
                        $task_id = $POSTS_ARRAY['task_id'];
                        $this->tasks_model->update($data,$task_id);
